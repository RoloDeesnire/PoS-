...
namespace POSales
{
    public partial class POSReport : Form
    {
        ...
        public void LoadCriticalItems(string sql)
        {
            try
            {
                ReportDataSource rptDS;
                string reportPath = Path.Combine(Application.StartupPath, "Reports", "rptCriticalItems.rdlc");
        ...
        public void LoadCriticalStock(string sql)
        {
            try
            {
                ReportDataSource rptDS;
                string reportPath = Path.Combine(Application.StartupPath, "Reports", "rptCriticalStock.rdlc");
                ...
                using (SqlConnection connection = new SqlConnection(dbcon.myConnection()))
                {
                    ...
                    {
                        DataSet1 ds = new DataSet1();
                        da.Fill(ds, "dtCriticalStock");

                        rptDS = new ReportDataSource("DataSet1", ds.Tables["dtCriticalStock"]);
                        reportViewer1.LocalReport.DataSources.Add(rptDS);
                    }
                }
        ...
        public void LoadExpiry(string sql)
        {
            try
            {
                ReportDataSource rptDS;
                string reportPath = Path.Combine(Application.StartupPath, "Reports", "rptExpiry.rdlc");
                ...
                using (SqlConnection connection = new SqlConnection(dbcon.myConnection()))
                {
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter(sql, cn))
                    {
                        ...
                        rptDS = new ReportDataSource("DataSet1", ds.Tables["dtExpiry"]);
                        reportViewer1.LocalReport.DataSources.Add(rptDS);
                    }
                }
