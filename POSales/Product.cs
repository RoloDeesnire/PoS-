...
namespace POSales
{
    public partial class Product : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT p.pcode, p.barcode, p.pdesc, b.brand, c.category, p.price, p.reorder, p.expiry FROM tbProduct AS p INNER JOIN tbBrand AS b ON b.id = p.bid INNER JOIN tbCategory AS c on c.id = p.cid WHERE CONCAT(p.pdesc, b.brand, c.category) LIKE '%" + txtSearch.Text + "%'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            productModule.txtPcode.Enabled = true;
            productModule.btnSave.Enabled = true;
            productModule.btnUpdate.Hide();
